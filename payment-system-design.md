...
* Payment Cards (credit, debit...) are used for payment into our system
* Third Party PSP (Payment System providers) are used for card processing
* Cards are not stored in Our system
* Our System supports only one currency
* Our System expects 1,000,000 transactions daily (1M/day in transactions)
* Our System supports A Payout flow (since this is an Amazon Pay kind of system where sellers are also included in the flow of transactions)
* Our System must be able to handle reconcilliation and settlemenent of any wrong/inconsistent data(transaction), therefore system stores states of transaction in flow.
...
* Our System must be reliable and fault-tolerant (failed payment need to be handled)
* Reconcilliation Process must be added for Failed payment and processing between internal systems(Payment system, accounting system, PSP)
...
* Exact one Delivery (Idempotency)
* Consistency
* Security
...
* *__Single Receiver__* ~ which can be implemeted by a shared queue, where multiple subscribers can receive messages respectively in the image below service1 and service2 both process M1

* *__Multiple Receiver__* ~ Almost same as single receiver but service1 and service2 process only messages meant for each services
...
1. *__At Most Once__* ~ Using an Idempotency check, for commnication between clients (web applications) an idempotency key is usually a a key generated by the client and expires after a certain number of time , usually a UUID added to the header , from an API standpoint idemepotency means clients can make the same call repeatedly and pricue the same result
    ...
    * Incremental retry
    * Exponential retry => **THIS IS WIDELY RECOMMENDED**
